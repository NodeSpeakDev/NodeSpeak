...
contract ForumCommunityManager {
    ...
    function incrementPostCount(uint32 communityId) external validCommunity(communityId) {
        communities[communityId].postCount++;
    }

    // Verificar que un topic sea válido para una comunidad (llamado desde el contrato de posts)
    function validateTopicForCommunity(uint32 communityId, string memory topic) external view validCommunity(communityId) returns (bool) {
        return communityTopics[communityId][topic];
    }
